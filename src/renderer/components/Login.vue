            ...
            >
                ...
                <el-form-item>
                    <el-button
                        ...
                        >Connect</el-button
                    >
                    <small style="color: #f3f3f3"
                        >Remember to terminate msfrpcd before starting server
                        <code style="background-color: #232e3b"
                            >&gt; kill msfrpcd</code
                        ></small
                    >
                </el-form-item>
...
export default {
    ...
    data() {
        ...
        let checkURL = (rule, value, callback) => {
            ...
            if (!port) {
                return callback(
                    new Error(
                        "Please provide the port number along with ip:[port]"
                    )
                );
            } else {
                callback();
        ...
        return {
            ...
            rules: {
                ...
                password: [
                    {
                        required: true,
                        message: "Metasploit password",
                        trigger: "blur",
                    },
                ],
                host: [{ validator: checkURL, trigger: "blur" }],
            },
            dialogText: "",
            showDialog: false,
            activeStep: 0,
    ...
    methods: {
                        ...
                        setTimeout(() => {
                            ...
                            if (res.result === "unsuccessful") {
                                ...
                                this.$message({
                                    showClose: true,
                                    message:
                                        "Network error, please check if msfrpcd is running and on which protocol, if all fine then double check port host:[port]",
                                    type: "error",
                                    duration: 0,
                                });
                            } else {
                                ...
                                if (res.result === "success") {
                                    ...
                                    let pathTo = this.$electron.remote.app.getPath(
                                        "home"
                                    );
                                    fs.mkdir(
                                        pathTo + `/kage`,
        ...
        async startServer() {
            await exec(
                "cd / && cd root && cd kage && msfconsole -r msfrpc.rc",
                (error, stdout, stderr) => {
                    console.log(error);
                    console.log(stdout);
                    console.log(stderr);
            ...
            response = await this.executeServerCommands(
                `msfrpcd -U ${username} -P ${password} -p 55552`
            );
            if (response === 0) {
                this.activeStep++;
            } else {
